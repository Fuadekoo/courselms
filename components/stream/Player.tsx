  ...
  const handleQualityChange = React.useCallback(
    (quality: QualityLevel) => {
      ...
      if (isHlsSource && hlsRef.current) {
        ...
        // For non-HLS videos with quality options
        const selectedQuality = qualities.find((q) => q.value === cq);
        if (selectedQuality) {
          // Update the source based on quality
          if (type === "local") {
            generateSecureUrl(selectedQuality.url).then((url) => {
              if (url) {
                setSecureVideoUrl(url);
            ...
            setSecureVideoUrl(selectedQuality.url);
          }
        }
      }
    },
    [isHlsSource, hlsLevels, qualities, type, generateSecureUrl]
  );
